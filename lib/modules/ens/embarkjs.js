...
__embarkENS.setProvider = function (config) {
  ...
  EmbarkJS.onReady(() => {
    web3.eth.net.getId()
      ...
      self.ens = new EmbarkJS.Blockchain.Contract({abi: config.registryAbi, address: registryAddress});
      self.registrar = new EmbarkJS.Blockchain.Contract({abi: config.registrarAbi, address: config.registrarAddress});
      self.resolver = new EmbarkJS.Blockchain.Contract({abi: config.resolverAbi, address: config.resolverAddress});
    })
      .catch(err => {
...
__embarkENS.resolve = function (name, callback) {
  ...
  return this.ens.methods.resolver(node).call((err, resolverAddress) => {
    ...
    let resolverContract = new EmbarkJS.Blockchain.Contract({abi: this.resolverInterface, address: resolverAddress});
